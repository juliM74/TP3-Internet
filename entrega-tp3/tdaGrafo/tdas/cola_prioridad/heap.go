...
const _FACTOR_REDIMENSIONAMIENTO = 2
const _REDIMEN_REDUCIR = 4
const _FACTOR_REDUCIR = 2
...
func CrearHeap[T comparable](cmp func(T, T) int) ColaPrioridad[T] {
	return nuevoHeap[T](_CAPACIDAD_INICIAL, cmp)
}
...
func (heap *colaConPrioridad[T]) hayQueAgrandar() (bool, int) {
	...
	if heap.cant == capacidad {
		return true, capacidad * _FACTOR_REDIMENSIONAMIENTO
	}
	return false, 0
}
...
func (heap *colaConPrioridad[T]) hayQueAchicar() (bool, int) {
	capacidad := cap(heap.datos)
	if capacidad > _CAPACIDAD_INICIAL && heap.cant*_REDIMEN_REDUCIR <= capacidad {
		return true, capacidad / _FACTOR_REDUCIR
	}
	return false, 0
}
