...
func TestDiccionarioClaveDefault(t *testing.T) {
	t.Log("Prueba sobre un ABB vacío que si justo buscamos la clave que es el default del tipo de dato, " +
		"sigue sin existir")
	dic := TDADiccionario.CrearABB[string, string](cmpString)
	...
	dicNum := TDADiccionario.CrearABB[int, string](cmpInt)
...
func TestUnElement(t *testing.T) {
	...
	dic := TDADiccionario.CrearABB[string, int](cmpString)
	dic.Guardar("A", 10)
...
func TestReemplazoDatoHopscotch(t *testing.T) {
	...
	dic := TDADiccionario.CrearABB[int, int](cmpInt)
	for i := 0; i < 500; i++ {
		dic.Guardar(i, i)
	}
	for i := 0; i < 500; i++ {
...
func TestABB_BorrarBasico(t *testing.T) {
	...
	require.True(t, dic.Pertenece("vaca"))

	// Borrar uno y verificar
	require.EqualValues(t, "guau", dic.Borrar("perro"))
	require.False(t, dic.Pertenece("perro"))
	require.EqualValues(t, 2, dic.Cantidad())
	require.PanicsWithValue(t, "La clave no pertenece al diccionario", func() { dic.Borrar("perro") })

	// Borrar otro
	require.EqualValues(t, "miau", dic.Borrar("gato"))
	require.False(t, dic.Pertenece("gato"))
	require.EqualValues(t, 1, dic.Cantidad())
	require.PanicsWithValue(t, "La clave no pertenece al diccionario", func() { dic.Obtener("gato") })

	// Borrar el último
	require.EqualValues(t, "moo", dic.Borrar("vaca"))
	require.False(t, dic.Pertenece("vaca"))
	require.EqualValues(t, 0, dic.Cantidad())
	require.PanicsWithValue(t, "La clave no pertenece al diccionario", func() { dic.Borrar("vaca") })
}
...
func TestConClavesStructs(t *testing.T) {
	...
	cmpBasico := func(a, b basico) int {
		if a.a != b.a {
			return strings.Compare(a.a, b.a)
		}
		return a.b - b.b
	}
	cmpAvanzado := func(a, b avanzado) int {
		if a.w != b.w {
			return a.w - b.w
		}
		if cmpBasico(a.x, b.x) != 0 {
			return cmpBasico(a.x, b.x)
		}
		if cmpBasico(a.y, b.y) != 0 {
			return cmpBasico(a.y, b.y)
		}
		return strings.Compare(a.z, b.z)
	}
	dic := TDADiccionario.CrearABB[avanzado, int](cmpAvanzado)
...
func TestPruebaIterarTrasBorrados(t *testing.T) {
	t.Log("Prueba de caja blanca: Esta prueba intenta verificar el comportamiento del ABB cuando " +
		"queda vacío tras varios borrados. El iterador debería estar al final, y luego funcionar bien si se agrega un elemento.")
